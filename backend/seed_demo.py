...
KNOWLEDGE_DOCUMENTS = [
    {
        ...
        "category": "it.vpn",
        "chunks": [
            "公司 VPN 无法连接时按以下顺序排查：1. 确认已连接外网；2. 检查 VPN 客户端是否为最新版本（错误码 769 通常表示目标不可达，809 表示连接被服务端拒绝）；3. 确认账号未被锁定，密码是否过期。仍无法连接请记录错误码并提交工单，附上设备型号、操作系统和网络环境。",
            "Windows 配置 VPN 步骤：设置 -> 网络和 Internet -> VPN -> 添加 VPN 连接，服务器地址填 vpn.company.com，类型选 L2TP/IPsec，用户名密码使用企业账号。Mac 用户在系统设置 -> 网络 -> VPN 中添加相同配置。",
        ],
    },
    {
        ...
        "chunks": [
            ...
            "收不到邮件时先检查：1. 是否被自动归档或垃圾邮件拦截；2. 客户端是否处于离线模式；3. 邮箱容量是否已满（超过 90% 建议归档历史邮件）。",
        ],
    },
    {
        ...
        "category": "it.account",
        "chunks": [
            "忘记密码可在登录页点击「忘记密码」，通过绑定手机号或企业微信扫码自助重置。SSO 账号锁定后需等待 15 分钟自动解锁，或联系 IT 服务台人工解锁。",
            "密码策略：至少 10 位，包含大小写字母、数字与特殊字符，90 天强制更换，不可与最近 5 次密码相同。",
        ],
    },
    {
        ...
        "category": "it.printer",
        "chunks": [
            "安装共享打印机：开始菜单 -> 设置 -> 蓝牙和其他设备 -> 打印机 -> 添加设备，选择 \\\\print-server\\printer-001。驱动缺失时从驱动站下载对应型号驱动，安装后打印测试页验证。",
    ...
    },
    {
        ...
        "chunks": [
            "办公软件通过软件中心自助安装，覆盖 Office、浏览器、压缩工具与会议客户端。安装企业软件需要管理员权限，请联系 IT 管理员开通。",
            "禁止安装未授权软件。许可证管理：部门许可证池不足时，先回收离职员工授权再申请新授权。",
        ],
    },
    {
        ...
        "category": "it.network",
        "chunks": [
            "断网排查顺序：1. 检查网线/无线连接状态；2. 查看其他设备是否同样断网（判断是否区域故障）；3. ipconfig /release 后 /renew 重新获取 IP；4. ping 网关与 8.8.8.8 判断内网/外网问题。",
            "Wi-Fi 连接不上时先忘记网络重新连接，确认选择了正确的 SSID（公司 5G 频段）并输入正确密码。办公区信号弱可切换到有线网口。",
        ],
    },
    {
        ...
        "chunks": [
            "电脑故障报修前先尝试：1. 重启；2. 外接显示器判断是否为屏幕故障；3. 检查电源与散热。报修工单需提供资产编号、故障现象、发生时间与紧急程度。",
            "硬件维修周期：笔记本 3 个工作日，台式机 1 个工作日，显示器 3 个工作日。维修期间可申请备用机。",
        ],
    },
    {
        ...
        "chunks": [
            "系统权限申请流程：1. 填写权限申请表（系统名称、所需权限、事由、期限）；2. 直属上级审批；3. IT 管理员开通并审计留痕。",
            "高危权限（管理员、批量导出、财务审批）需要部门负责人与 IT 管理员双重审批，权限默认最小化授予并定期复核。",
        ],
    },
]
...
def main() -> None:
    ...
    for key, value in counts.items():
        print(f"   - {key}: {value}")
    print()
    print("演示账号（开发令牌，AUTH_MODE=dev）：")
    print('  uv run python -m backend.issue_dev_token demo customer-1 --role helpdesk-customer')
    print('  uv run python -m backend.issue_dev_token demo agent-1 --role helpdesk-agent')
    print('  uv run python -m backend.issue_dev_token demo admin-1 --role helpdesk-it-admin')
    print()
