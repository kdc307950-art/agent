...
KNOWLEDGE_DOCUMENTS = [
    ...
    },
    {
        ...
        "category": "it.account",
        "chunks": [
            "忘记密码可在登录页点击「忘记密码」，通过绑定手机号或企业微信扫码自助重置。SSO 账号被锁定后需等待 15 分钟自动解锁，账号被停用或离职账号回收请联系 IT 服务台处理。邮箱账号密码错误时，先确认使用的是统一身份认证密码，再检查账号是否锁定或密码是否过期。MFA 收不到验证码时，先确认绑定手机号/企业微信可用，再由 IT 管理员核验身份。重置后仍登不进去，请确认新密码未违反策略并检查 Caps Lock。",
    ...
    },
    {
        ...
        "category": "it.network",
        "chunks": [
            "办公区断网、网速慢、DNS 无法解析、视频会议卡顿、丢包或延迟高时排查顺序：1. 检查网线/无线连接状态；2. 查看其他设备是否同样断网（判断是否区域故障）；3. ipconfig /release 后 /renew 重新获取 IP；4. ping 网关与 8.8.8.8 判断内网/外网问题；5. 必要时重启路由器或接入设备；6. 记录丢包率、延迟和 DNS 返回结果，提交给 IT 服务台。",
            "Wi-Fi 连不上、提示密码错误或交换机端口未启用时，先确认终端网卡、网线、交换机端口状态和正确的 SSID（公司 5G 频段）。代理服务器异常时检查系统代理配置；办公区无线信号弱可切换到有线网口，或联系 IT 排查该区域接入点。",
        ],
    },
    {
