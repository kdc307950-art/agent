...
describe('CopilotPanel', () => {
  ...
  it('生成期间禁止重复点击', async () => {
    let resolveGen: (value: CopilotGenerateResult) => void = () => {}
    vi.spyOn(copilotApi, 'generateCopilot').mockImplementation(
      () =>
        new Promise<CopilotGenerateResult>((resolve) => {
          resolveGen = resolve
        }),
    )
    render(<CopilotPanel ticketId="t-1" expectedVersion={1} enabled onAdopt={() => {}} />)
    ...
    resolveGen({ run_id: 'run-1', draft, idempotent_replay: false })
  })

  it('202 running 时显示"正在生成"', async () => {
    vi.spyOn(copilotApi, 'generateCopilot').mockResolvedValue({
      status: 'running',
      run_id: 'run-1',
    } as never)
